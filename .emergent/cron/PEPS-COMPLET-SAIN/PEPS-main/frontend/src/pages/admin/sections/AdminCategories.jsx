...
export default function AdminCategories() {
  ...
  const remove = async (row) => {
    const ok = await confirm({
      title: `Delete category "${row.name_en}"?`,
      confirmLabel: "Delete",
      cancelLabel: "Cancel",
      destructive: true,
    });
  ...
  return (
    <div data-testid="admin-categories">
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-ash last:border-0" data-testid={`category-row-${r.slug}`}>
                ...
                <td className="px-4 py-3 text-right whitespace-nowrap">
                  <button data-testid={`category-edit-${r.slug}`} onClick={() => setEditing(r)} className="p-2 hover:text-nordfjord">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-nordfjord/60 backdrop-blur-xl px-4">
          <div className="w-full max-w-md bg-white border border-ash rounded-lg shadow-2xl" data-testid="category-dialog">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 rounded-sm border border-ash bg-white px-3 py-2 text-sm font-mono focus:outline-none focus:border-nova"
                />
              </div>
