    ...
    <div data-testid="admin-menus">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="rounded-full bg-nova text-nordfjord font-mono text-xs uppercase tracking-[0.2em] px-5 py-2.5 inline-flex items-center gap-2"
        >
          <Plus size={14} /> New
        </button>
      </div>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-ash last:border-0" data-testid={`menu-row-${r.slug}`}>
                ...
                <td className="px-4 py-3 text-right whitespace-nowrap">
                  <button data-testid={`menu-edit-${r.slug}`} onClick={() => setEditing(JSON.parse(JSON.stringify(r)))} className="p-2 hover:text-nordfjord">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-nordfjord/60 backdrop-blur-xl px-4 py-8 overflow-y-auto">
          <div className="w-full max-w-3xl bg-white border border-ash rounded-lg shadow-2xl my-auto" data-testid="menu-dialog">
                ...
                {[{ k: "name_en", l: "Name (EN)" }, { k: "name_fr", l: "Nom (FR)" }, { k: "slug", l: "Slug" }].map((f) => (
                  <div key={f.k}>
                    ...
                    <input
                      ...
                      className="w-full mt-1 rounded-sm border border-ash bg-white px-3 py-2 text-sm focus:outline-none focus:border-nova"
                    />
                  </div>
                ))}
                <div>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="grid sm:grid-cols-3 gap-2 flex-1">
                          <input placeholder="Label EN" value={it.label_en || ""} onChange={(e) => setItem(i, { label_en: e.target.value })}
                            className="rounded-sm border border-ash bg-white px-2 py-1.5 text-sm focus:outline-none focus:border-nova" />
                          ...
                          <input placeholder="/catalog" value={it.url || ""} onChange={(e) => setItem(i, { url: e.target.value })}
                            className="rounded-sm border border-ash bg-white px-2 py-1.5 text-sm font-mono focus:outline-none focus:border-nova" />
                        </div>
                      ...
                      <div className="flex flex-wrap items-center gap-5 mt-2 pl-6">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={it.published !== false} onChange={(e) => setItem(i, { published: e.target.checked })}
                            className="w-3.5 h-3.5 accent-nova" data-testid={`menu-item-published-${i}`} />
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={!!it.open_new_tab} onChange={(e) => setItem(i, { open_new_tab: e.target.checked })}
                            className="w-3.5 h-3.5 accent-nova" />
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-ash">
              ...
              <button data-testid="menu-save" disabled={busy} onClick={save}
                className="rounded-full bg-nova text-nordfjord px-5 py-2 font-mono text-xs uppercase tracking-[0.2em] disabled:opacity-60">
                Save
              </button>
            </div>
          </div>
