...
- **NOWPayments Batch Payout**
  ...
  - Endpoint `GET /api/admin/affiliates/payouts/export.csv` — fichier d'import NOWPayments Mass Payouts **au gabarit officiel** (`PayoutsTemplate.csv`) : Ticker, Wallet Address, ExtraId, Amount in crypto (6 décimales, arrondi vers le bas), Fiat amount et Fiat currency laissés vides (le montant en jetons fait foi), Payout description portant l'id du versement. Pour les `ready` + `queued_manual`. Sans BOM, fins de ligne LF, comme le gabarit.
...
## Item 3.2 — Février 2026 (session fork, suite : seuil minimum payout affilié)
- **Nouvelle env var** `AFFILIATE_PAYOUT_MIN_CAD` (défaut **25.00 CAD**) — les commissions cumulées d'un affilié sous ce seuil pour une période NE créent PAS de ligne dans `affiliate_payouts` : les référrals restent en `approved` avec `payout_id=None` et roulent au cycle suivant. Évite de gaspiller les frais gas crypto (USDT/USDC ERC-20) sur des micro-payouts.
- **Notification bilingue (FR/EN)** — Helper `_defer_affiliate_payout_below_threshold()` insère un doc `affiliate_payout_deferrals` (unique idx sur `(affiliate_id, period)`) puis met en file un email via `email_outbox` : sujet localisé, montant cumulé, seuil, montant restant à atteindre, note pédagogique sur les frais gas. Choix de la langue via `preferred_lang` de l'affilié.
- **Idempotence** — Le scheduler mensuel ou un rejeu manuel via `POST /admin/affiliates/payouts/run` ou `/force-run` ne re-notifie PAS un affilié déjà notifié pour la même période. Response API enrichie : `payouts_created`, `payouts_deferred`, `threshold_cad`, `detail[]` contient les entrées `deferred:true / notified:bool` pour les affiliés reportés.
- **Tests pytest** — `/app/backend/tests/test_affiliate_payout_threshold.py` (3 tests, 100% passing) : (1) LOW 12$ deferred + HIGH 40$ paid, (2) DB side-effects (pas de payout row, referrals intacts, deferral row unique, email queued), (3) rerun idempotence (`notified=False` au 2ᵉ run, 1 seule deferral row).
- **Aucune régression** — Suite `test_sprint5_affiliate_e2e.py` 15/15 pass. Les erreurs `test_iter22` et `test_affiliate_h4` observées sont pré-existantes (rate-limit login + status legacy `paid_manual`), sans lien avec ce changement.

## État des tâches P1 après audit
- **TTL commandes PENDING** : DÉJÀ IMPLÉMENTÉ. `cancel_stale_unpaid_orders()` cancelle les statuts `awaiting_etransfer`/`awaiting_crypto` après `UNPAID_ORDER_TTL_HOURS` (défaut 24h) ; `_cancel_order_side_effects()` restocke via `_restock_order_items()` ; watchdog `_unpaid_orders_watchdog()` s'exécute toutes les heures + drainage `_drain_unpaid_order_batches`. Aucune action requise.
- **Pre-Deploy Ops** : ACTIONS UTILISATEUR REQUISES (hors code) — rotation des 4 secrets tiers dans les consoles fournisseurs, ajout `APP_ENV=production` au pod prod, purge `demo.affilie@fironova.com` de la DB prod.


## Quick Restock — Février 2026 (session fork, suite 3 : restock atomique + audit)
- **Nouvel endpoint** `POST /api/admin/products/{id}/restock` (body `{deltas:[{variant_id?, quantity>0, note?}], reason?}`) — restock atomique multi-variantes via `$inc` MongoDB. Impossible de perdre du stock en cas d'actions concurrentes. Rejette `quantity<=0` (Pydantic `gt=0`). Réponse `{ok, product, applied[], skipped[]}` avec raisons de skip (`variant_not_found`, `update_failed`).
- **Audit trail persisté** — Collection `stock_movements` indexée sur `(product_id, created_at desc)`. Chaque mouvement stocke : `delta, stock_before, stock_after, variant_name, admin_email, reason, created_at`. Endpoint `GET /api/admin/products/{id}/stock-history?limit=50` (max 500).
- **Hook restock-notify** — Si stock passe de 0 → >0, déclenche `_maybe_notify_restock(product_id, variant_id)` pour envoyer les emails "prévenez-moi" aux abonnés existants.
- **UI Quick Restock (AdminProducts)** — Nouveau bouton vert `PackagePlus` par ligne (testid `restock-<slug>`). Modal dédié affiche par variante : Current (RO), Add (input +N + boutons rapides `+10/+25/+50/+100`), New total (calculé live). Champ Reason optionnel. Banner de confirmation "N units will be added across X variant(s)". Bouton `VIEW HISTORY` bascule sur table des 50 derniers mouvements (date, variant, delta, before→after, admin, reason). Le workflow **absolute value** dans le drawer d'édition reste dispo pour les cas rares (correction inventaire).
