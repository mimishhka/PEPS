...
Use `COOKIE_SAMESITE=lax` when the frontend and API are same-site. Keep `CORS_ORIGIN_REGEX` empty unless preview domains are necessary and tightly constrained. Never set `CORS_ORIGINS=*` or `TRUSTED_PROXIES=0.0.0.0/0`.
